...
async function add(req, res) {
    try {
        ...
        const flight = await em.findOne(Flight, { id: flight_id }, { populate: ['destino'] });
        if (!flight) {
...
async function findUserReservations(req, res) {
    try {
        const em = orm.em.fork();
        const userId = req.user.id;
        // 1. La consulta ahora carga todas las relaciones anidadas que necesitamos.
        const reservations = await em.find(Reservation, { usuario: userId }, {
...
async function cancelReservation(req, res) {
    try {
        ...
        const reservation = await em.findOne(Reservation, { id: reservationId, usuario: userId }, { populate: ['flight'] });
        if (!reservation) {
        ...
        const fechaVuelo = new Date(reservation.flight.fechahora_salida);
        if (fechaVuelo < new Date()) {
